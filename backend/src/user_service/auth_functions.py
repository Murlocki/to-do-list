...
def create_refresh_token(data: dict, expires_delta: timedelta = None):
    ...
    logger.info(f"Refresh token created for user: {data['sub']}")
...
def create_new_access_token(email: str):
    return create_token(data={"iss": "auth-service", "sub": email, "jti": str(uuid.uuid4())})
...
def is_token_about_to_expire(exp_time: datetime, threshold: int = 300):
...
def verify_and_refresh_token(token: str):
    try:
        ...
        about_to_expire: bool = is_token_about_to_expire(exp_time)

        if about_to_expire or datetime.now() > exp_time:
            ...
            if "refresh_token" in session:
                ...
                new_token: str = create_new_access_token(session["refresh_token"])
                if new_token:
                ...
                new_access_token = create_new_access_token(payload['sub'])
...
def verify_and_refresh_refresh_token(refresh_token: str):
    try:
        ...
        if not session:
            ...
            return None
        new_access_token = create_new_access_token(email)
